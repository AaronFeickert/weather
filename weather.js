...
async function getLiveData() {
  ...
  let response = await fetch(
    "http://online.saiawos.com/MSP/ios/webgetjson.php?buster=" +
      new Date().getTime(),
    { cache: "reload" },
  );
  ...
  if (
    ...
    data["utc"].slice(data["utc"].indexOf(":") + 1, data["utc"].indexOf(" ")) ==
      new Date().getMinutes()
  ) {
  ...
  document.getElementById("wind_speed").textContent =
    round(convert_kts_to_mph(wind_speed), 1) + " mph";
  ...
  document.getElementById("temperature").textContent =
    temperature.substring(0, temperature.indexOf("(")) + " F";
  let apparent =
    data["heatIndex"] != "N/A" ? data["heatIndex"] : data["windChill"];
...
async function getWeather(latitude, longitude) {
  ...
  document.getElementById("forecast_caption").textContent =
    "Forecast weather (" +
...
async function getUV(latitude, longitude) {
  ...
  response = await fetch(
    "https://data.epa.gov/efservice/getEnvirofactsUVHourly/LATITUDE/" +
      latitude +
      "/LONGITUDE/" +
      longitude +
      "/JSON",
